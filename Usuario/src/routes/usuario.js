...
route.post("/cadastro", (req, res) => {
    bcrypt.hash(req.body.senha, cfg.salt, (erro, result) => {
        ...
        .save()
        .then((result) => {
            res.status(201).send({ output: `Cadastro realizado com sucesso`, payload: result });
        })
        .catch((erro) =>
  ...
  route.post('/altera-senha', verifica_token, (req, res) => {
    ...
    bcrypt.compare(req.body.senha, usuario.senha, (erro, same) => {
        if (erro)
            return res
            .status(500)
            .send({ output: `Erro ao validar a senha` });
        if (!same) return res.status(400).send({ output: `Senha inválida` });
        bcrypt.hash(req.body.novaSenha, cfg.salt, (erro, result) => {
            Usuario.findByIdAndUpdate(usuario._id, {senha: result}, {new: false}, (erro, usuario) => {
                if (erro) 
                    res.status(500).send({erro});
                res.status(200).send({success: true, output: `Atualizado com sucesso`});
            });
        });
        });
    });
